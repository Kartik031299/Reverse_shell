...
while True:  # An infinite loop to execute multiple commands recieved from server
	data=s.recv(1024) # recieving data in form of bytes from server(1024 B chunk)

	if data[:4].decode("utf-8") == "quit":   # when quit command is entered at server end
		s.close()           # close socket
		sys.exit()          # exit from python script
